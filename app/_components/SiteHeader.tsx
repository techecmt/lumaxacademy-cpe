...
import { policySections } from "../data/studentpolicies";

const ESSENTIAL_RESOURCES_PATH =
...
const navItems: NavItem[] = [
  ...
  },
  {
    ...
    children: [
      { label: "Student Support", href: "/student-affairs" },
      {
      ...
      ...policySections.map((section) => ({
        label: section.title,
        href: `${ESSENTIAL_RESOURCES_PATH}#${section.id}`,
        group: "Policies & Resources",
      })),
    ],
  },
...
export default function SiteHeader() {
  ...
  const socials = useMemo(
    () => [
      {
        label: "Facebook",
        href: "https://www.facebook.com/profile.php?id=61589531251092",
        icon: FaFacebookF,
      },
      {
        label: "Instagram",
        href: "https://www.instagram.com/lumax.academy/",
        icon: FaInstagram,
      },
      {
        label: "TikTok",
        href: "https://www.tiktok.com/@lumax.academy4",
        icon: SiTiktok,
      },
      {
        label: "YouTube",
        href: "https://www.youtube.com/@LumaxAcademy",
        icon: SiYoutube,
      },
    ],
    [],
        ...
        <div className="mx-auto max-w-7xl px-4">
          <div className="flex items-center justify-between gap-6 py-2 text-sm text-slate-600">
            ...
            <div className="flex items-center gap-2">
              {socials.map(
                (s: { label: string; href: string; icon: IconType }) => (
                  <a
                    key={s.label}
                    href={s.href}
                    target="_blank"
              ...
              <div className="flex items-center gap-1">
                {navItems.map((item) => {
                              ...
                              <div className="relative overflow-hidden bg-linear-to-br from-[#102b50] via-[#193764] to-[#0c203b] p-6 text-white">
                                ...
                                <div className="relative">
                                  ...
                                  <p className="mt-2 text-xs leading-relaxed text-white/60">
                                    Industry-relevant, part-time diplomas
                                    created for working learners and ambitious
                                    career switchers.
                                  </p>
                  ...
                  if (item.children?.length) {
                    const wideMenu = item.children.length > 6;
                    return (
                      <div key={item.label} className="group relative">
                        <button
                          type="button"
                        ...
                        <div
                          className={[
                            "pointer-events-none absolute left-0 top-full z-50 pt-2 opacity-0 translate-y-1 transition group-hover:pointer-events-auto group-hover:opacity-100 group-hover:translate-y-0 group-focus-within:pointer-events-auto group-focus-within:opacity-100 group-focus-within:translate-y-0",
                            wideMenu
                        ...
                        >
                          <div className="rounded-2xl border border-(--border) bg-white p-2 shadow-[0_18px_50px_-24px_rgba(2,6,23,0.45)] ring-1 ring-black/5">
                            <div
                              className={
                                wideMenu ? "grid grid-cols-2 gap-x-1" : ""
                              }
                            >
                              {item.children.map((child, childIndex) => {
                                ...
                                const showGroup =
                                  !!child.group &&
                                  child.group !==
                                    item.children?.[childIndex - 1]?.group;
                                return (
                                  <Fragment key={child.label}>
                                    {showGroup ? (
                                      <div className="col-span-2 border-t border-(--border) px-3 pb-1 pt-3 text-[11px] font-extrabold uppercase tracking-wider text-slate-400">
                                        {child.group}
                                      </div>
                                    ) : null}
                                    <Link
            ...
            <div className="relative overflow-hidden bg-linear-to-br from-[#193764] via-[#1f477c] to-[#10213d] px-4 pb-4 pt-3.5 text-white">
              ...
              <div className="relative mt-3 flex items-center gap-2">
                ...
                >
                  <svg
                    viewBox="0 0 24 24"
                    className="h-4.5 w-4.5 fill-(--brand)"
                    aria-hidden
                  >
                          ...
                          <div className="grid gap-1 pl-2">
                            {item.children.map((child, childIndex) => {
                              ...
                              return (
                                <Fragment key={child.label}>
                                  {showGroup ? (
                                    <div className="px-1 pb-0.5 pt-2 text-[10px] font-extrabold uppercase tracking-wider text-slate-400">
                                      {child.group}
                                    </div>
                                  ) : null}
                                  <Link
